...
function map_worker() {
   self.addEventListener('message',function(evt) {
      ...
      for (var t = 0; t < triangles; ++t) {
         ...
         //
         x0 = Math.round((w-1)*(x0-xmin)/(xmax-xmin))
         x1 = Math.round((w-1)*(x1-xmin)/(xmax-xmin))
         x2 = Math.round((w-1)*(x2-xmin)/(xmax-xmin))
         y0 = Math.round((h-1)*(y0-ymin)/(ymax-ymin))
         y1 = Math.round((h-1)*(y1-ymin)/(ymax-ymin))
         y2 = Math.round((h-1)*(y2-ymin)/(ymax-ymin))
         //
         ...
         if (y2 != y1) {
            for (var y = y1; y <= y2; ++y) {
               x12 = Math.round(0.5+x1+(y-y1)*(x2-x1)/(y2-y1))
               z12 = z1+(y-y1)*(z2-z1)/(y2-y1)
               x02 = Math.round(0.5+x0+(y-y0)*(x2-x0)/(y2-y0))
               ...
               while (x != x02) {
                  ...
                  if (z > map[(h-1-y)*w+x]) {
                     map[(h-1-y)*w+x] = z
                     var iz = Math.round(255*(z-zmin)/(zmax-zmin))
         ...
         if (y1 != y0) {
            for (var y = y0; y <= y1; ++y) {
               x01 = Math.round(0.5+x0+(y-y0)*(x1-x0)/(y1-y0))
