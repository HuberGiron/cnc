...
(function() {
    ...
    var merge_toolpaths = function() {
        ...
        if (t1.path.length == 0) {
            mod.paths = [t2.path]
            mod.tools = [mod.tool2.value]
            mod.dpi = t2.dpi
            mod.width = t2.width
            mod.height = t2.height
            mod.depth = t2.depth
        } else if (t2.path.length == 0) {
            mod.paths = [t1.path]
            mod.tools = [mod.tool1.value]
            mod.dpi = t1.dpi
            mod.width = t1.width
            mod.height = t1.height
            mod.depth = t1.depth
        } else {
            mod.paths = [t1.path, t2.path]
            mod.tools = [mod.tool1.value, mod.tool2.value]
            mod.dpi = t1.dpi
            mod.width = t1.width
            mod.height = t1.height
            mod.depth = Math.max(t1.depth, t2.depth)
        }
    }
